...
class IPMTnetwork(nn.Module):
    def __init__(self, layers=50, classes=2, shot=1, reduce_dim=384, \
        ...
        if self.with_transformer:
            ...
            self.transformer = IPMTransformer(embed_dims=reduce_dim, num_points=9,shot=self.shot)
    ...
    def forward(self, x, s_x, s_y, y):
        ...
        query_feat_list,qry_outputs_mask_list,sup_outputs_mask_list = self.transformer(query_feat, y.float(), aug_supp_feat, s_y.clone().float(),init_mask.detach())
